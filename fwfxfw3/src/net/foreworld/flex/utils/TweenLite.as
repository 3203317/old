...
package net.foreworld.flex.utils
{
	...
	public final class TweenLite
	{
		public static var version:Number = 5.9;
		...
		public function initTweenVals():void {
			...
			var p:String, val:*, valChange:Number; //For looping (for p in this.vars)
			if (this.target is Array) {
				...
				for (p in this.vars) {
					...
					if (p == "delay" || p == "ease" || p == "overwrite" || p == "onComplete" || p == "onCompleteParams" || p == "runBackwards" || p == "onUpdate" || p == "onUpdateParams" || p == "autoAlpha" || p == "onStart" || p == "onStartParams" || p == "easeParams" || p == "mcColor" || p == "type") { //"type" is for TweenFilterLite, and it's an issue when trying to tween filters on TextFields which do actually have a "type" property.
						...
						if (val != null && val != "") { //In case they're actually trying to remove the colorization, they should pass in null or "" for the tint
							_endColor.color = val;
						}
						...
						if (this.target.hasOwnProperty(p)) {
							if (typeof(this.vars[p]) == "number") {
								valChange = val - this.target[p];
							} else {
								valChange = Number(val);
							}
							...
							this.extraTweens[p] = {o:this.target, s:0, c:0, v:val}; //classes that extend this one (like TweenFilterLite) may need it (like for blurX, blurY, and other filter properties)
						}
					}
				}
			}
