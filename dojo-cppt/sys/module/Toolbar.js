/**
 * 作者：黄鑫
 * 日期：2013-03-11
 * 描述：模块Toolbar
 */
define(["dojo/_base/declare",
	"dojo/_base/lang",
	"dojo/_base/connect",
	"dijit/Toolbar",
	"dijit/form/Button",
	"dijit/Tooltip",
	"dojo/i18n!./nls/Toolbar",
	"dijit/layout/ContentPane"], function($declare, $lang, $connect,$Toolbar,$Button,$Tooltip,$i18n,$ContentPane) {
	return $declare("internal.sys.module.Toolbar", $ContentPane, {
		postCreate : function() {
			this.inherited(arguments);
			this._addButtons();
		},
		startup:function(){
			this.inherited(arguments);
			this._addSubscribes();
			this._addEvents();
		},
		_addSubscribes : function() {
			this._handle1 = $connect.subscribe("/internal/sys/module/Toolbar/EditBtn/disable/", this, "_onDisableEditBtn");
			this._handle2 = $connect.subscribe("/internal/sys/module/Toolbar/DelBtn/disable/", this, "_onDisableDelBtn");
		},
		_addEvents: function(){
			this._connect1 = $connect.connect(this._addBtn, "onClick", this, "_showAddDlg");
			this._connect2 = $connect.connect(this._editBtn, "onClick", this, "_showEditDlg");
			this._connect3 = $connect.connect(this._delBtn, "onClick", this, "_showDelDlg");
		},
		_removeHandles: function(){
			this._handle1.remove();
			this._handle2.remove();

			this._connect1.remove();
			this._connect2.remove();
			this._connect3.remove();
		},
		_addButtons: function(){
			this._toolbar = new $Toolbar({},document.createElement("div"));
			this.domNode.appendChild(this._toolbar.domNode);

			this._addBtn = new $Button({
				iconClass: 'dijitEditorIcon dijitEditorIconNewPage',
				label: $i18n.btnLabAdd
			});
			this._editBtn = new $Button({
				iconClass: 'dijitEditorIcon dijitEditorIconWikiword',
				disabled:true,
				label: $i18n.btnLabEdit
			});
			this._delBtn = new $Button({
				iconClass: 'dijitEditorIcon dijitEditorIconDelete',
				disabled:true,
				label: $i18n.btnLabDel
			});

			this._toolbar.addChild(this._addBtn);
			this._toolbar.addChild(this._editBtn);
			this._toolbar.addChild(this._delBtn);
		},
		_onDisableEditBtn: function($boolean){
			this._editBtn.set("disabled",$boolean);
		},
		_onDisableDelBtn: function($boolean){
			this._delBtn.set("disabled",$boolean);
		},
		_showAddDlg: function(){
			console.log("显示模块新增对话框");
		},
		_showEditDlg: function(){
			console.log("显示模块编辑对话框");
		},
		_showDelDlg: function(){
			console.log("显示模块删除确认对话框");
		},
		destroy: function(){
			this._removeHandles();
			this.destroyDescendants();
			this.inherited(arguments);
		}
	});
});