...
define(["dojo/_base/connect",
	...
	"internal/sys/group/RoleRelatGrid",
	"internal/sys/group/UnRoleRelatGrid"], function($connect,$declare,$lang,$ajax,$FormDialog,$Button) {
	return $declare($FormDialog,{
		...
		fid: "internal_sys_group_RoleForm",
		title: "角色分配",
		href: "sys/group/roleForm.html",
...
//			$ajax({
//				async : true,
//				content: { groupid: __data_3.i.id, roles: dojo.toJson(__itemArr) },
...
//			this.hide();
		...
		_saveData1: function(){
			...
			$ajax({
				async : true,
				content: { groupid: __data_3.i.id, roles: dojo.toJson(__sels), type: "add" },
				url : this.vpath +"testData/user_o0.json?orgId=",
				callback : function(){
		...
		_saveData2: function(){
			...
			$ajax({
				async : true,
				content: { groupid: __data_3.i.id, roles: dojo.toJson(__sels), type: "del" },
				url : this.vpath +"testData/user_o0.json?orgId=",
				callback : function(){
		...
		_searchUnRole: function(){
			var __data_3 = this._grid.getSelectItem();
			var __param = {
				domainid: dijit.byId(this.fid +"_domainidText").key,
				rolename: dijit.byId(this.fid +"_rolenameText").getValue(),
		...
		_draw: function(){
			...
			__searchBtn.on("click", $lang.hitch(this, "_searchUnRole"));
		...
		_setTitle: function(){
			switch(this.model){
			case "Update":
				this.set("title", "角色分配    [组："+ this._grid.getSelectItem().i.groupname+"]");
				break;
			}
		},
		_setData: function(){
			if(this.model == "Create"){
				...
				/* 为右Grid赋值 */
				dijit.byId(this.fid).reset()
				dijit.byId(this.fid +"_domainidText").set("key",0);
				this._searchUnRole();
			}
			this._valid();
		}
