...
define(["dojo/_base/connect",
	...
	return $declare($FormDialog,{
		fid: "internal_sys_user_PassForm",
		title: "",
		href: "sys/user/passForm.html",
		style: { height: "124px", width: "280px" },
		_grid: null,
		_store: null,
		_tree: null,
...
//					this._store.newItem(__newItem);
		...
		_draw: function(){
			...
			dijit.byId(this.fid +"_valpassText").validator = function($value, $constraints){
				if(dijit.byId($constraints.other).isValid()){
					var __newPass = $value;
					var __valPass = dijit.byId($constraints.other).value;
					return (__newPass == __valPass);
				}else{
					return false;
				}
			}
		...
		_setTitle: function(){
			...
			case "Update":
				...
				this.set("title","重置密码 [姓名："+ __data_3.i.username +"]");
				break;
			}
		},
		_setData: function(){
			if(this.model == "Create"){
				...
				dijit.byId(this.fid +"_newpassText").set("value", "");
				dijit.byId(this.fid +"_valpassText").set("value", "");
			}
			this._valid();
		}
	});
