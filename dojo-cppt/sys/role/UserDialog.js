...
define(["dojo/_base/connect",
	...
	"internal/sys/role/UserRelatGrid",
	"internal/sys/role/UnUserRelatGrid",
	"internal/sys/role/ResourceTree"], function($connect,$declare,$lang,$ajax,$FormDialog,$Button) {
	return $declare($FormDialog,{
		...
		fid: "internal_sys_role_UserForm",
		title: "用户分配",
		href: "sys/role/userForm.html",
		...
		_saveData1: function(){
			...
			$ajax({
				async : true,
				content: { roleid: __data_3.i.id, users: dojo.toJson(__sels), type: "add" },
		...
		_saveData2: function(){
			...
			$ajax({
				async : true,
				content: { roleid: __data_3.i.id, users: dojo.toJson(__sels), type: "del" },
				url : this.vpath +"testData/user_o0.json?orgId=",
				callback : function(){
		...
		_setTitle: function(){
			switch(this.model){
			case "Update":
				this.set("title", "用户分配    [角色："+ this._grid.getSelectItem().i.rolename+"]");
				break;
			}
		},
		_setData: function(){
