...
!function($) {
   ...
   $.fn.validation.defaults = {
        validRules : [
            {name: 'remote', validate: remoteValid, defaultMsg: '远程校验'},
    ...
    function remoteValid(value){
    	...
    	var url = $field.attr("remote-url");
    	var paramsMethod = $field.attr("remote-param-method");
    	var result = false;
    	if($.trim(url)!='' && $.trim(paramsMethod)!=''){
    		try{
	    		var data = eval(paramsMethod+"(value,$field)");
	    		$.ajax({
	    			url:url,
	    			data:data,
	    			async:false,
