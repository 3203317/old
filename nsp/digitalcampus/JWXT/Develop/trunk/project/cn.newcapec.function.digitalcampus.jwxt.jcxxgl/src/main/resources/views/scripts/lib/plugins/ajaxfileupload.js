...
//				newElement.appendTo(form);
			...
			creatUploadFormByClas : function(id, fileElementId, data) {
				//create form	
				var formId = 'jUploadForm' + id;
				var fileId = 'jUploadFile' + id;
				var form = jQuery('<form  action="" method="POST" name="'
...
//								+ data[i] + '" />').appendTo(form);
				...
				jQuery(oldElement).attr('id', fileId);
				//如果class为多个文件，复制多个
				if(jQuery(oldElement).length>0){
					oldElement.each(function(i,e){
						$(e).before(newElement[i]);
					});
				}
...
//				newElement.appendTo(form);
			...
			ajaxFileUpload : function(s) {
				...
				//做判断是根据id取file还算根据class取
				var form;
				var _datas = typeof (s.data) == 'undefined' ? false : s.data;
				//默认是根据id取
				if(s.fileType && s.fileType=='class'){
					form = jQuery.creatUploadFormByClas(id, s.fileElementId,_datas);
				}else{
					form = jQuery.createUploadForm(id, s.fileElementId,	_datas);
				}
