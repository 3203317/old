package cn.newcapec.function.citycard.platform.privilege.rest;
...
import cn.newcapec.function.citycard.platform.privilege.biz.DepartmentService;
import cn.newcapec.function.citycard.platform.privilege.biz.RoleService;
import cn.newcapec.function.citycard.platform.privilege.biz.UserRoleService;
import cn.newcapec.function.citycard.platform.privilege.biz.UserService;
import cn.newcapec.function.citycard.platform.privilege.model.User;
import cn.newcapec.function.citycard.platform.privilege.utils.UserTool;
...
public class UserResource extends PrivilegeResource {
    ...
    public void loginUI(BaseRequest request, BaseResponse response) {
            String url = "/function/citycard/platform/privilege/v1.0/user/pagelet/loginUI.html";
...
//	        }
                ...
                if (StringUtils.isNotBlank(password)) {
                    if (user != null) {
                        if (user.getPassword().equals(password)) {
                            ...
                            WebUtils.getResponse(request.getOrginRequest()).sendRedirect(this.getContextPath()+"/restful/portalProxyService/tradition/index");
                            return;
    ...
    public void userListUI(BaseRequest request, BaseResponse response) {
            String url = "/function/citycard/platform/privilege/v1.0/user/pagelet/user_list.html";
    ...
    public void userListGrid(BaseRequest request, BaseResponse response) {
            String url = "/function/citycard/platform/privilege/v1.0/user/pagelet/user_list_grid.html";
        /* 查询列表 */
    ...
    public void addUserUI(BaseRequest request, BaseResponse response) {
        String url = "/function/citycard/platform/privilege/v1.0/user/pagelet/addUserUI.html";
    ...
    public void updateUserUI(BaseRequest request, BaseResponse response) {
        String url = "/function/citycard/platform/privilege/v1.0/user/pagelet/updateUserUI.html";
        try {
            if (null != getJsonObject()) {
...
//				User user = userService.get(id);
                JSONUtils.getMorpherRegistry().registerMorpher(
                        new DateMorpher(new String[]{"yyyy-MM-dd",
                                "yyyy-MM-dd HH:mm:ss"}));
                User user = (User) JSONObject.toBean(getJsonObject(), User.class);
                if (null != user) {
                    // 判断用户账号是否存在
                    Msg msg = new Msg();
                    msg.setSuccess(true);
    ...
    public void openDistributionUI(BaseRequest request, BaseResponse response) {
        ...
        String url = "/function/citycard/platform/privilege/v1.0/user/pagelet/distributionUserRoleUI.html";
        // 获取界面的reloid
        String userid = JSONTools.getString(getJsonObject(), "userid");
