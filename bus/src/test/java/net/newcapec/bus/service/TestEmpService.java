...
public class TestEmpService {
	static final Logger _log = Logger.getLogger(TestEmpService.class);
	static EmpService _service = null;
	static final SimpleDateFormat _sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");

	@BeforeClass
	public static void setUpBeforeClass() throws Exception {
		ApplicationContext __ctx = new ClassPathXmlApplicationContext("classpath:applicationContext.xml");
		_service = (EmpServiceImpl) __ctx.getBean("empServiceImpl");
	}
	...
	public void findEmpById() {
		...
		ResultMapper<Emp> __mapper = _service.findEmpById(empCode, customerUnitCode);
		boolean __bool = __mapper.getStatus().equals(ResultMapper.SUCCESS);
		if (!__bool) {
			_log.info(__mapper.getMsg());
			Assert.assertEquals(__bool, true);
		}

		_log.info("客户名称：" + __mapper.getT().getEmpName());
	}
	...
	public void findEmpList() {
		...
		for (Emp __emp_3 : __mapper.getList()) {
			_log.info(__emp_3.getEmpCode() + "		" + __emp_3.getIncomeDate());
		}

		_log.info("当前：" + page.getCurrent() + " 本页：" + __mapper.getList().size() + " 总数：" + __mapper.getCount());
	}
